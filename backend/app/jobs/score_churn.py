...
def score_latest() -> dict:
    ...
    snapshot = date.today()
    frame = build_snapshot(engine, snapshot, label_days=0)
    if frame.empty:
        return {"scored": 0}
    model = load_model()
    meta = load_metadata()
    ...
    with Session(engine) as session:
        session.execute(
            delete(ChurnScore).where(
                ChurnScore.snapshot_date == snapshot, ChurnScore.model_version == version
            )
        )
...
if __name__ == "__main__":
    print(json.dumps(score_latest(), indent=2))
