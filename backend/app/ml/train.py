...
def temporal_split(df):
    ...
    if len(dates) < 2:
        raise RuntimeError("Need at least two snapshot dates for temporal validation")
...
def train() -> dict:
    ...
    artifact_uri = upload_artifact(model_path, f"models/{version}/churn_model.joblib")
    upload_artifact(model_path, "models/latest/churn_model.joblib")
    upload_artifact(meta_path, "models/latest/model_metadata.json")
