...
def analyze_binary(
    ...
    pc = control_conversions / control_n
    pt = treatment_conversions / treatment_n
    diff = pt - pc
    se = math.sqrt(pt * (1 - pt) / treatment_n + pc * (1 - pc) / control_n)
    ...
    return {
        ...
        "z_stat": float(z),
        "p_value": float(p),
        "difference_ci": [diff - zcrit * se, diff + zcrit * se],
