...
async def get_current_account(credentials:HTTPAuthorizationCredentials|None=Depends(bearer), db:AsyncSession=Depends(get_db), redis:Redis=Depends(get_redis))->Account:
    ...
    account=await db.scalar(select(Account).where(Account.id==UUID(payload["sub"]),Account.is_active.is_(True)))
    if account is None: raise HTTPException(status_code=401,detail="Account not found")
    return account
